...
const Cliente = () => {
    ...
    const filteredClientes = Array.isArray(nombre) ? nombre.filter(cliente =>
    ...
    const handleDelete = async () => {
        try {
            ...
            setNombres(nombre.filter(cliente => cliente.id !== clienteToDelete.id));
            closeModal();
    ...
    const handleAddCliente = () => {
        navigate('/principal/cliente/add'); 
    };

    const handleEditCliente = (id) => {
        navigate(`/principal/cliente/edit/${id}`); 
    };
    ...
    return (
        <div className="w-4/5 mx-auto p-10 border border-gray-300 rounded-lg bg-beige-100">
            ...
            <div className="flex justify-between items-center mb-6">
                <h1 className="text-3xl font-bold">Lista Cliente</h1>
            </div>
            <div className="flex justify-between items-center mb-6">
                <button className="bg-blue-500 text-white px-4 py-2 rounded-lg hover:bg-blue-600 transition duration-300" onClick={handleAddCliente}>
                    Agregar Cliente
                </button>
                ...
                <table className="border-collapse md:border-separate border border-blue-200 w-full bg-white shadow-lg">
                    ...
                    <tbody>
                        {filteredClientes.map((cliente, index) => (
                            <tr key={cliente.id} className="hover:bg-gray-100 transition duration-300 text-center">
                                <td className="border border-black-200 px-4 py-2">{index + 1}</td>
                                <td className="border border-black-200 px-4 py-2">{cliente.nombre}</td>
                                <td className="border border-black-200 px-4 py-2">{cliente.telefono}</td>
                                <td className="border border-black-200 px-4 py-2">{cliente.email}</td>
                                <td className="border border-black-200 px-4 py-2">{cliente.direccion}</td>
                                <td className="border border-black-200 px-4 py-2 flex justify-around">
                                    <button className="text-blue-500 hover:text-blue-700 transition duration-300" onClick={() => handleEditCliente(cliente.id)}>
                                        <Edit size={25} />
                                    </button>
                                    <button className="text-red-500 hover:text-red-700 transition duration-300" onClick={() => openModal(cliente)}>
                                        <Trash size={25} />
                                    </button>
                                </td>
                            </tr>
            ...
            >
                ...
                <p>¿Estás seguro de que deseas eliminar al Cliente {clienteToDelete?.nombre}?</p>
